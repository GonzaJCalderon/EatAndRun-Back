...
CREATE TABLE public.daily_menu (
    ...
    image_url text,
    CONSTRAINT daily_menu_for_role_check CHECK (((for_role)::text = ANY ((ARRAY['usuario'::character varying, 'empresa'::character varying])::text[])))
);
...
CREATE TABLE public.order_items (
    ...
    CONSTRAINT order_items_item_type_check CHECK (((item_type)::text = ANY ((ARRAY['daily'::character varying, 'extra'::character varying, 'tarta'::character varying, 'fijo'::character varying, 'skip'::character varying])::text[]))),
...
ALTER TABLE ONLY public.orders
    ADD CONSTRAINT orders_delivery_id_fkey FOREIGN KEY (delivery_id) REFERENCES public.users(id);
