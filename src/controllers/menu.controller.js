...
export const getSemanaActualController = async (req, res) => {
  try {
    ...
    // 🧼 Normalizar días habilitados (puede venir jsonb, json o string)
    const parseDias = (value) => {
      const fallback = { lunes:false, martes:false, miercoles:false, jueves:false, viernes:false };
    ...
    return res.json({
      ...
      yaCerro,
      dias_habilitados: parseDias(s.dias_habilitados),
    });
  } catch (err) {
...
export async function putSemana(req, res) {
  try {
    ...
    // 1) normalizar entradas a "YYYY-MM-DD"
    let fi = isoToDateOnly(fecha_inicio);
    // 2) forzar lunes
    fi = mondayOf(fi).format('YYYY-MM-DD');
    // 3) fin = viernes (semana Lun–Vie)
    const ff = fridayOf(fi).format('YYYY-MM-DD');
    // 4) cierre: si no mandan, viernes; si mandan, normalizado
    const ci = cierre ? isoToDateOnly(cierre) : ff;

    // 5) upsert por semana_inicio
